...
export class Runtime {
    ...
    async installPackages(rscript, manifest, opts = {}) {
        ...
        await writeFile(join(this.dataDir, 'install_packages.R'), INSTALL_SCRIPT, 'utf8');
        const proc = spawn(rscript, ['--no-echo', '--file=install_packages.R', join(this.dataDir, 'manifest-runtime.json')], {
            cwd: this.dataDir,
            env: { ...process.env, R_LIBS_USER: this.libraryDir },
            windowsHide: true,
        });
        ...
        const stillMissingMatch = /STILL_MISSING:\s*(.*)/.exec(out);
        if (timedOut)
            throw new Error(`package installation timed out after ${timeoutMs}ms`);
        if (stillMissingMatch && stillMissingMatch[1].trim() !== '') {
            throw new Error(`these packages are still missing after installation: ${stillMissingMatch[1].trim()}\n${err.slice(-4000)}`);
        }
...
// Generated per run; jsonlite reads the manifest, BiocManager pins Bioc 3.20.
