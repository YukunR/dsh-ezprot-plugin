...
const installScriptPath = () => join(packageDir, 'r', 'setup', 'install_packages.R');
...
export function wireKillOnAbort(proc, signal) {
    if (!signal)
        return;
    const onAbort = () => {
        try {
 ...
 * (R-4.4.0, R_4.4.0, ...).
 */
export function candidateScriptPaths(runtimeDir) {
    const list = [];
    if (process.platform === 'win32') {
        ...
        for (const root of ['D:\\R', 'C:\\Program Files\\R'])
            list.push({ path: root, directory: true });
    }
    else {
        list.push({ path: '/usr/local/bin/Rscript', directory: false }, { path: '/usr/bin/Rscript', directory: false }, { path: '/opt/homebrew/bin/Rscript', directory: false }, { path: '/opt/R', directory: true });
    }
    return list;
}
...
export class Runtime {
    ...
    async detectRscript() {
        if (this.config.rscript)
            return this.config.rscript;
    ...
    async installR(opts = {}) {
        ...
        if (!downloaded)
            throw new Error(`cannot download R ${R_VERSION} installer from any mirror`);
        ...
        await new Promise((resolvePromise, reject) => {
            ...
            proc.on('close', (code) => {
                clearTimeout(timer);
                if (code === 0 && existsSync(join(installDir, 'bin', 'Rscript.exe')))
                    resolvePromise();
                else
                    reject(new Error(`R installer exited with code ${code}`));
            });
        });
    ...
    async installPackages(rscript, manifest, opts = {}) {
        const log = opts.onLog ?? (() => { });
    ...
    async missingPackages(rscript, manifest) {
     ...
     */
    async verifyRuntime(rscript, manifest, opts = {}) {
        const log = opts.onLog ?? (() => { });
        ...
        await writeFile(join(this.dataDir, 'manifest-runtime.json'), JSON.stringify(runtimeManifest));
        const proc = spawn(rscript, [join(packageDir, 'r', 'setup', 'check_runtime.R'), join(this.dataDir, 'manifest-runtime.json')], {
            cwd: this.dataDir,
            env: { ...process.env, R_LIBS_USER: this.libraryDir },
            windowsHide: true,
        });
    ...
    async restoreSnapshot(snapshotPath, opts = {}) {
        const log = opts.onLog ?? (() => { });
        const timeoutMs = opts.timeoutMs ?? 20 * 60 * 1000;
        ...
        if (timedOut)
            throw new Error(`snapshot extraction timed out after ${timeoutMs}ms`);
        if (!existsSync(this.libraryDir))
            throw new Error(`snapshot extracted but no library dir found at ${this.libraryDir}: ${err.slice(-2000)}`);
