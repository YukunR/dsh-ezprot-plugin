...
import { Runtime, DEFAULT_DOCKER_IMAGE } from './runtime.js';
import { Backgrounds, ORGANISMS, packageDir } from './backgrounds.js';
import { Project, preflight, toContainerPath, writeGeneratedSampleInfo } from './pipeline.js';
...
export class ProteomicsService {
    ...
    async environmentSetup(opts = {}) {
        const { action, snapshotPath, backend, onLog, signal } = opts;
        const log = onLog ?? (() => { });
        const manifest = await this.loadManifest();
        ...
        if (backend === 'docker') {
            ...
            const verify = await this.runtime.dockerVerify(image, { onLog, signal });
            if (!verify.ok) {
        ...
        if (!rscript) {
            ...
            rscript = await this.runtime.installR({ onLog, signal });
        }
        else {
        ...
        if (action === 'restore_snapshot') {
            ...
            await this.runtime.restoreSnapshot(snapshotPath, { onLog, signal });
        ...
        await this.runtime.installPackages(rscript, manifest, { onLog, timeoutMs: 45 * 60 * 1000, signal });
        await this.runtime.setState({ backend: 'local' });
        return this.environmentStatus();
    }
    // ── backgrounds ───────────────────────────────────────────────────────────
    ...
    async preflightProject(opts) {
        ...
        if (qc.nonNumericSampleColumns.length > 0)
            lines.push(`WARNING: ${qc.nonNumericSampleColumns.length} non-numeric column(s) are being treated as samples: ${qc.nonNumericSampleColumns.join(', ')} — confirm the sample list with the user (annotation flags like "Reverse"/"Potential contaminant" must not be samples; re-import with explicit sample columns if needed)`);
    ...
    async verifyRuntimeReport(opts = {}) {
        const manifest = await this.loadManifest();
        ...
        const probe = await this.runtime.verifyRuntime(rscript, manifest, { signal: opts.signal });
        if (probe.ok) {
        ...
        return this.withLock(project.dir, async () => {
            ...
            if (step === 'enrich' || step === 'gsea' || step === 'all') {
                ...
                if (!bs.go || !bs.kegg) {
                    ...
                    await this.backgrounds.ensure(state.organism, { onLog: log, backend, dockerImage });
                }
            }
    ...
    async stepSummary(project, step) {
        switch (step) {
            case 'normalization':
