...
export function toContainerPath(p) {
    return toHostPath(p).replace(/^[A-Za-z]:/, '');
}
/**
 ...
 */
export function dockerMountArgs(projectDir, backgroundsDir) {
    return [
        '--mount', `type=bind,source=${toHostPath(projectDir)},target=/workspace`,
        '--mount', `type=bind,source=${toHostPath(backgroundsDir)},target=${toContainerPath(backgroundsDir)}`,
    ];
}
export class Project {
    ...
    async runStep(runtime, step, opts = {}) {
        ...
        if (opts.backend === 'docker') {
            // The container runs the project's own run.R (scripts were copied at
            // preflight) with the project dir bind-mounted at /workspace and the
            // annotation background cache at its drive-less host path (see
            // dockerMountArgs / toContainerPath).
            mkdirSync(join(runtime.dataDir, 'backgrounds'), { recursive: true });
            proc = spawn('docker', [
                'run', '--rm',
                '-w', '/workspace',
                ...dockerMountArgs(this.dir, join(runtime.dataDir, 'backgrounds')),
