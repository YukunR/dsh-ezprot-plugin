...
export class ProteomicsService {
  ...
  async preflightProject(opts: {
    ...
    if (qc.nonNumericSampleColumns.length > 0) lines.push(`WARNING: ${qc.nonNumericSampleColumns.length} non-numeric column(s) are being treated as samples: ${qc.nonNumericSampleColumns.join(', ')} — confirm the sample list with the user (annotation flags like "Reverse"/"Potential contaminant" must not be samples; re-import with explicit sample columns if needed)`)
  ...
  async verifyRuntimeReport(opts: { signal?: AbortSignal } = {}): Promise<string> {
    const manifest = await this.loadManifest()
