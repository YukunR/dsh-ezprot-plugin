...
function runtimeManifest(ctx: PackagesContext, manifest: PackageManifest) {
  return {
    cran: manifest.cran,
    bioc: manifest.bioc,
...
export function installedPackages(ctx: PackagesContext, rscript: string): Promise<string[]> {
...
export async function installPackages(ctx: PackagesContext, rscript: string, manifest: PackageManifest, opts: { onLog?: LogSink; timeoutMs?: number; signal?: AbortSignal } = {}): Promise<void> {
...
export async function missingPackages(ctx: PackagesContext, rscript: string, manifest: PackageManifest): Promise<string[]> {
 ...
 */
export async function verifyRuntime(ctx: PackagesContext, rscript: string, manifest: PackageManifest, opts: { onLog?: LogSink; timeoutMs?: number; signal?: AbortSignal } = {}): Promise<{ ok: boolean; failures: string[]; tail: string }> {
  const log: LogSink = opts.onLog ?? (() => {})
