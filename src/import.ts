...
export interface InspectResult {
  ...
  nRows: number
  preview: string[][]
  missing: { nan: number; blank: number; zero: number }
...
export function parseTable(text: string, sep = '\t'): ParsedTable {
  const lines = text.split(/\r?\n/)
  if (lines.length === 0) return { header: [], rows: [], missing: { nan: 0, blank: 0, zero: 0 } }
  ...
  for (const line of lines.slice(1)) {
    ...
    for (const c of cells) {
      const v = c.trim()
      if (v === '' || v.toLowerCase() === 'na') missing.blank++
      else if (v.toLowerCase() === 'nan') missing.nan++
      else if (v === '0' || v === '0.0') missing.zero++
    }
  }
...
export function looksNumeric(values: string[]): number {
  ...
  for (const v of values) {
    const t = v.trim()
    if (t === '' || t.toLowerCase() === 'nan' || t.toLowerCase() === 'na') continue
    total++
    if (/^[-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?$/.test(t)) numeric++
  }
  return total === 0 ? 0 : numeric / total
}

/** Classify every column of a raw table with heuristics (candidates only — the user confirms). */
export function classifyColumns(header: string[], rows: string[][]): ColumnClass[] {
  const columnValues = (idx: number): string[] => rows.slice(0, 300).map((r) => r[idx] ?? '').filter((v) => v !== undefined)
  return header.map((name, idx) => {
    const h = name.trim()
    const values = columnValues(idx)
    const ratio = looksNumeric(values)
    if (ID_HEADER.test(h)) return { name, role: 'id', reason: 'header matches protein-id vocabulary', numericRatio: ratio }
    if (GENE_HEADER.test(h)) return { name, role: 'gene', reason: 'header matches gene-symbol vocabulary', numericRatio: ratio }
    if (DESC_HEADER.test(h)) return { name, role: 'desc', reason: 'header matches description vocabulary', numericRatio: ratio }
    if (ANNOTATION_HEADER.test(h)) return { name, role: 'annotation', reason: 'header matches quality/annotation vocabulary', numericRatio: ratio }
    if (SAMPLE_HEADER.test(h)) return { name, role: 'sample', reason: 'header matches quantification vocabulary', numericRatio: ratio }
    if (ratio >= 0.8) {
...
export function inferGroupNames(sampleNames: string[]): string[] {
  const infer = (name: string) => String(name).replace(/[_\-.]\d+$/, '')
  return [...new Set(sampleNames.map(infer))]
}

/** Inspect a raw file: convert Excel to TSV when needed, classify, preview. */
export async function inspectRawFile(inputFile: string, runtime: Runtime, opts: { sheet?: string } = {}): Promise<InspectResult> {
  ...
  if (IS_XLSX.test(inputFile)) {
    ...
    if (sheetsMatch) sheets = sheetsMatch[1].split('|').map((s) => s.trim()).filter(Boolean)
    convertedFrom = out
    file = out
  }
...
export async function tidyRawFile(inputFile: string, runtime: Runtime, outputDir: string, opts: TidyOptions): Promise<TidyResult> {
  let file = inputFile
  if (IS_XLSX.test(inputFile)) {
    const importDir = join(runtime.dataDir, 'imports')
    mkdirSync(importDir, { recursive: true })
  ...
  const idx = (name: string | undefined): number => (name === undefined ? -1 : header.indexOf(name))
  const idIdx = idx(opts.idColumn)
  if (idIdx < 0) throw new Error(`id column '${opts.idColumn}' not found in header: ${header.join(', ')}`)
  ...
  for (let i = 0; i < sampleIdx.length; i++) {
    if (sampleIdx[i] < 0) throw new Error(`sample column '${opts.sampleColumns[i]}' not found in header`)
  }
  ...
  for (const row of table.rows) {
    ...
    if (seen.has(acc)) duplicateAccessions++
    seen.add(acc)
    const samples = sampleIdx.map((i) => {
      const v = (row[i] ?? '').trim()
      if (v === '' || v.toLowerCase() === 'nan' || v.toLowerCase() === 'na') return 'NaN'
      if (missingZero && (v === '0' || v === '0.0' || v === '-0' || v === '-0.0')) return 'NaN'
      return v
    })
    outRows.push([
      acc,
  ...
  if (outRows.length === 0) throw new Error('no data rows remained after tidy — check the id column selection')
