...
const PROMPT_SECTION = [
  ...
  '2. proteomics_environment action=status checks the R runtime; action=setup installs R and any missing packages automatically (one-time, 10-20 min). setup runs as a BACKGROUND JOB: it returns a jobId — poll job_output(jobId) repeatedly and narrate progress to the user in plain language (packages downloading / installing), then finish with action=status (and action=verify after a first-time setup).',
...
export function apply(ctx: Context, config: Config) {
  ...
  const registerImage = async (absPath: string, name?: string): Promise<ImageRefLike | null> => {
    try {
  ...
  const getJobs = () => (ctx.get('jobs') as unknown as import('./tools.js').JobsProvider | undefined) ?? undefined
