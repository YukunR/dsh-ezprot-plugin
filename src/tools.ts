...
function formatEnvironment(status: EnvironmentReport): string {
  ...
  for (const [org, s] of Object.entries(status.organisms)) {
    lines.push(`${org} backgrounds: GO ${s.go ? 'ready' : 'missing (built on demand)'}, KEGG ${s.kegg ? 'ready' : 'missing (built on demand)'}`)
  }
...
export function buildToolDefinitions(service: ProteomicsService, registerImage?: ImageRegistrar, getJobs?: GetJobs): ToolDefinitionOptions[] {
  return [
...

    {
      ...
      description:
        'Check or build the GO/KEGG annotation backgrounds for an organism (human/mouse/rat). Every organism is built once from KEGG REST + UniProt on first use and cached permanently; builds need internet once per organism, and all downstream enrichment/GSEA steps use the local files and never touch the network.',
      parameters: {
...

    {
      ...
      description:
        'Set the project comparisons after the user has confirmed them. Lightweight: updates the project state and regenerates main.R, no QC re-run. Requires a completed proteomics_preflight.',
      parameters: {
...

    {
      ...
      description:
        'Return a consolidated digest of a finished proteomics project: completed steps, per-comparison DE counts, top GO/KEGG terms and GSEA sets.',
      parameters: {
        projectDir: { type: 'string', description: 'Project directory (absolute path).', required: true },
      },
      output: textOutput,
