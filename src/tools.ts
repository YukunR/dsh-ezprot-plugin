...
export function buildToolDefinitions(service: ProteomicsService, registerImage?: ImageRegistrar, getJobs?: GetJobs): ToolDefinitionOptions[] {
  return [
...

    {
      ...
      execute: async (args: any) => {
        ...
        if ((args.action ?? 'inspect') === 'inspect') {
          ...
          if (r.inferredGroups.length > 0) lines.push(`inferred groups from sample-like columns: ${r.inferredGroups.join(', ')}`)
          lines.push('preview (first 4 rows):')
          for (const row of r.preview) lines.push('  ' + row.join(' | '))
...

    {
      ...
      description:
        'Set the project comparisons after the user has confirmed them (ask_user_question with candidate pairs first — never guess comparisons). Lightweight: updates the project state and regenerates main.R, no QC re-run. Requires a completed proteomics_preflight.',
      parameters: {
