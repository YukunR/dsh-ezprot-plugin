...
import { DEFAULT_DOCKER_IMAGE, downloadFile, dockerRun, type DockerRunResult, type LogSink, type Runtime } from './runtime.js'
...
export class Backgrounds {
   ...
   */
  async ensure(organism: Organism, opts: BackgroundBuildOptions = {}): Promise<{ go: string; kegg: string }> {
    const log: LogSink = opts.onLog ?? (() => {})
  ...
  async buildKegg(organism: Organism, opts: BackgroundBuildOptions = {}): Promise<void> {
    const log: LogSink = opts.onLog ?? (() => {})
  ...
  async buildGo(organism: Organism, opts: BackgroundBuildOptions = {}): Promise<void> {
    const log: LogSink = opts.onLog ?? (() => {})
    const org = ORGANISMS[organism]
    ...
    if (opts.backend === 'docker') {
      ...
      if (dl.timedOut) throw new Error(`GO background download timed out for ${organism}`)
      if (dl.code !== 0) throw new Error(`GO background download failed for ${organism} (exit ${dl.code})\n${dl.tail.slice(-3000)}`)
   ...
   */
  private async runInDocker(dockerImage: string | undefined, rscriptArgs: string[], opts: { onLog?: LogSink; signal?: AbortSignal } = {}): Promise<DockerRunResult> {
...
export function rDownloadCommand(url: string, dest: string): string {
 ...
 */
export function backgroundScriptPath(name: string, backend: 'local' | 'docker'): string {
