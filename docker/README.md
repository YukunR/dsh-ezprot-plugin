...
`r/setup/install_packages.R` 与 `r/setup/check_runtime.R`（清单即单一事实源），
**构建完成即通过运行时探针**，否则 build 直接失败。发布后用户 `docker pull`
成品，使用侧完全不再依赖 CRAN/Bioc 镜像质量。
...
> Westlake CRAN archive。全部在 `r/setup/install_packages.R` 内按平台自动选择，
> Windows/macOS 路径不变。
>
...
> 该主机钉到已知 IPv4 再构建（`r/setup/install_packages.R` 内置重试 + Westlake 源码
> 回退，即使个别包失败也会自动收敛）：
> ```powershell
